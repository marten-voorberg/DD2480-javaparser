...
public class MethodResolutionLogic {
    ...
    private static boolean isAssignableMatchTypeParametersMatchingQName(ResolvedReferenceType expected, ResolvedReferenceType actual,
        ...
        for (int i = 0; i < expected.typeParametersValues().size(); i++) {
            ...
            boolean Type  = checkTypeVariable(expectedParam,actualParam,matchedParameters);
            boolean ReferenceType  = checkReferenceType(expectedParam,actualParam,matchedParameters);
            boolean WildCard = checkIsWildCard(expectedParam,actual,matchedParameters);
            if(CheckIfOneArgumentIsTrue(expectedParam.isReferenceType(),expectedParam.isTypeVariable(),expectedParam.isWildcard())){
                return CheckIfOneArgumentIsTrue(Type,ReferenceType,WildCard);
            }
            else {
    ...
    private static boolean checkReferenceType(ResolvedType expectedParam, ResolvedType actualParam, Map<String, ResolvedType> matchedParameters){
    ...
    private static boolean checkIsWildCard(ResolvedType expectedParam, ResolvedType actual, Map<String, ResolvedType> matchedParameters){
